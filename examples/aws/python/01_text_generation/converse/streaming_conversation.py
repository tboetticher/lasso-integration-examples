"""
Simple AWS Bedrock Converse streaming example through the Lasso proxy.

This example sends one user message and prints the model response as text
chunks arrive from the stream.
...
# 4. Call Bedrock Converse Stream through Lasso
try:
        ...
        messages=[
            {
                ...
                "content": [
                    {
                        "text": "Write a short paragraph about why streaming responses are useful."
                    }
                ],
            }
        ],
    ...
    logger.exception("Bedrock streaming request failed: %s", error)